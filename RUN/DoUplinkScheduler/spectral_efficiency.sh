...
until [ $NBUE -gt $2 ]; do
	echo FOR $NBUE USERS >> results/Spectral_efficiency.ods
	# bash until loop
#MT
	echo MT >> results/Spectral_efficiency.ods
	until [ $COUNT -gt $NUMSIM ]; do
	TOTALNAME=$FILE"_"$COUNT"_"$FILENAME"_MT_"$NBUE"U"$CELS"C"".sim"

	grep -i "^RX" results/$TOTALNAME |grep -iv "AM_RLC"  | awk '{print $8+5}'  > tmp
	grep -i "^RX" results/$TOTALNAME |grep -i "AM_RLC"  | awk '{print $4+5}'  >> tmp
./compute_spectral_efficiency.sh tmp  >> results/Spectral_efficiency.ods
	rm tmp

	 let COUNT=COUNT+1
	done
	COUNT=1
	echo AVERAGE >> results/Spectral_efficiency.ods
	echo  >> results/Spectral_efficiency.ods
	echo  >> results/Spectral_efficiency.ods	
#FME
	echo FME >> results/Spectral_efficiency.ods
	until [ $COUNT -gt $NUMSIM ]; do
...
./compute_spectral_efficiency.sh tmp  >> results/Spectral_efficiency.ods
	...
	echo  >> results/Spectral_efficiency.ods
#RR
	echo RR >> results/Spectral_efficiency.ods
	until [ $COUNT -gt $NUMSIM ]; do
